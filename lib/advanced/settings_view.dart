...
class _SettingsViewState extends State<SettingsView> {
  ...
  Widget _buildField(Map setting) {
  ...
  Widget _buildSelectField(Map setting) {
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton(
          ...
          onChanged: (final value) {
            _createSelectChangeHandler(setting);
          },
        ),
      ),
    );
  ...
  Widget _buildSwitchField(Map setting) {
    String name = setting['name'];
    bool value = _state?.map[name] ?? false;
    return InputDecorator(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton(
                ...
                onChanged: (String? value) {
                  if (value == null) return;
                  setState(() {
                    _loiteringDelay = int.tryParse(value) ?? 0;
                  });
                },
              ),
            ),
